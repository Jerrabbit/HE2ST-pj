...
## 最终指标总表：unfiltered rep1→rep2 全评测（2026-09-07）

> 统一口径：rep2 **全量 111,345 cells**、313 基因；log1p_zscore（统计量只训练集拟合）；PCC = 逐基因宏平均（313 基因各一个跨细胞 Pearson 再平均）；Top-k/AUROC 反归一化到 raw 计。best-epoch 与终值同用 rep2（无第三留出集，结果略乐观）。按 PCC 降序。

| 方法 | 版本/说明 | PCC | SPCC | cell_PCC | SSIM | Top-50 | AUROC |
|---|---|---|---|---|---|---|---|
| **Local+Global** | UNI2 CLS+Local(l1=112,l2=56) concat，原版 3 层 MLP | **0.3712** | 0.3071 | — | — | 0.588 | 0.759 |
| **SQUALL**（解码器头） | 自定义 313 头（frozen tokens→训练 decoder head） | 0.3348 | 0.2897 | 0.6735 | 0.4485 | 0.576 | 0.744 |
| **UNI2+MLP**（基线） | 单 CLS，原版 3 层 MLP | 0.3240 | 0.2838 | 0.6709 | 0.4467 | 0.578 | 0.737 |
| **GHIST** | +分割CE(核前景二值, w=0.1)，batch8 | 0.3132 | 0.2918 | 0.6329 | 0.3991 | 0.558 | 0.701 |
| **SpatialEx** | 超图 k 修复版（超边 7） | 0.2969 | 0.2685 | 0.6483 | 0.4298 | 0.563 | 0.726 |
| **Pixel2Gene**（cell） | 官方 ForwardSum 头（log1p 空间） | 0.2947 | 0.2695 | 0.6564 | 0.4388 | 0.555 | 0.722 |
| **Path2Space** | 冻结 CTransPath+重训官方 MLP 头 | 0.2835 | 0.2594 | 0.6476 | 0.4066 | 0.566 | 0.718 |
| **STFlow**（官方协议） | 去噪器+官方推理/超参 | 0.2696 | 0.2642 | 0.6162 | 0.4498 | 0.536 | 0.717 |
| **DeepPT**（R50 忠实版） | 官方 ResNet50-AE-MLP（AE 联合微调；AE 冻结版 0.2422 更低未用） | 0.2684 | 0.2516 | 0.6405 | 0.4053 | 0.562 | 0.712 |
| **ST-Net**（冻结） | DenseNet121 冻结 + 线性头（bias=均值初始化在 zscore 下≈0） | 0.2423 | 0.2348 | 0.6209 | 0.3712 | 0.550 | 0.699 |
| **BLEEP**（冻结） | resnet50 冻结+对比检索（raw-加权版中止，保留 0.1822；seed 0/1/2 复跑待补） | 0.1822 | 0.1728 | 0.5535 | 0.3388 | 0.495 | 0.639 |
| **Hist2ST**（官方配置） | 100ep ZINB+bake；seed 0/1/2 复跑待补 | 0.1818 | 0.1783 | 0.5628 | 0.3623 | 0.521 | 0.648 |
| **Phoenix 313 微调** | flow-only 微调（313 基因）→ ⏳ 全指标评测运行中（0.2055 待更新 cell/SSIM） | — | — | — | — | — | — |

> 注：LG 的 unf cell_PCC/SSIM 未在本轮留存（见 LG 节数值 0.3712/SPCC 0.3071 等）；BLEEP/Hist2ST 的 seed0-2 与 Phoenix313 全指标完成后回填替换。官方对齐"改进版"里仅 SpatialEx(k 修复) 微升被采用，DeepPT/Path2Space/BLEEP 的严格官方化实测不升反降 → 保留原版并在 §4.1 说明。
...
| **SQUALL（解码器头）** | **0.3348** | 0.2897 | 0.6735 | 0.4485 | 0.513 | 0.576 | 0.624 | 0.744 | ✅ 本轮补齐（unf，rep2 全量 111k） |
| **GHIST**（3-MSE，batch8） | **0.3122** | 0.2887 | 0.6245 | 0.3940 | 0.480 | 0.556 | 0.633 | 0.705 | ✅ 3-MSE 版完成；⚠️ +分割CE 版训练中（对照，见"官方对齐改进版"节） |
| **Hist2ST（官方配置）** | **0.1818** | 0.1783 | 0.5628 | 0.3623 | 0.408 | 0.521 | 0.613 | 0.648 | ✅ unf 官方配置（09-05 复核全评测） |
| **BLEEP（冻结）** | **0.1822** | 0.1728 | 0.5535 | 0.3388 | 0.385 | 0.495 | 0.590 | 0.639 | ✅ 本轮无种子；⚠️ seed 复跑进行中（核查 0.21） |
| **Phoenix 微调** | 0.2055 | 0.1585 | — | — | 0.455 | 0.496 | 0.525 | 0.610 | 0.2055 已有；无 cell_PCC/SSIM（就地微调版见"官方权重就地微调"节 0.2202） |

**本轮补齐（2026-09-06/07）**：SQUALL(解码器头) / BLEEP(冻结) / Hist2ST(官方配置) / GHIST(3-MSE) 四行的 unf **全评测**已补上（含 cell_PCC / SSIM / 全 Top-k）。BLEEP 0.1822 / Hist2ST 0.1818 为无固定种子单次运行；两者另做 **seed 0/1/2 固定种子复跑**（`scripts/run_seed_rerun.sh`，排在 GHIST+分割CE、Phoenix 313 全评测之后）核查能否回到旧 ~0.21 区间，完成后回填；GHIST 3-MSE 终值 0.3122 已入表，+分割CE 对照版训练中（见下节）。
...
#### 4.1 官方对齐改进版 + 特征归因（2026-09-07 更新，部分待收尾）

**A. 官方对齐"改进版"逐方法实测（unf，rep2 全量；与 §4 旧值对照）**
| 方法 | 改进点 | 新 PCC | 旧 PCC(原版) | 备注 |
|---|---|---|---|---|
| **SpatialEx** | 超图 k 语义修复（超边 7，非 8） | **0.2969** | 0.2944 | 微升；已采用 |
| **DeepPT(R50)** | AE 训后冻结 + 预训 20ep | 0.2422 | 0.2684 | ⚠️ **明显下降**：per-cell 下"AE 冻结+官方式预训"不如原联合微调 |
| **Path2Space** | 训练头补官方 bias_init | 0.2790 | 0.2835 | 略降（z-score 下 bias_init≈0，效果甚微） |
| **BLEEP** | 检索改官方 raw 空间加权 | 中止（val≈0.07） | 0.1822 | ⚠️ raw 加权在 z-score 协议下**退化**，已回滚，主表保留 0.1822 |
| **GHIST** | +分割CE（核前景二值, 权重0.1） | ⏳ 训练中 | 3-MSE **0.3122** | 对照版出数后定夺 |

**判定**：官方对齐改动里仅 SpatialEx(k 修复) 微升；DeepPT/Path2Space/BLEEP 的"严格官方化"在统一协议/per-cell 下**不升反降或退化为低值** → 主表取值建议：SpatialEx 用 0.2969；DeepPT/Path2Space/BLEEP 暂保留原值并在备注写明"官方对齐版实测更低/未采用"。

**B. 特征归因（同学特征 × 三种 MLP 头，均 rep2 全量、313 基因；vs 我们单 CLS 基线 0.3240）**
| 特征(dim) | 我们 3 层 MLP | RefMLP(Softplus)/log1p | bn2(2层256+BN) | 备注 |
|---|---|---|---|---|
| 我们 UNI2 CLS(1536) | **0.3240** | 0.3160 | 0.3186 | 基线 |
| 同学 UNI(3072)=Global+Local concat | 0.3684 | 0.3615 | 0.3641 | ≈ 我们的 LG/忠实特征 |
| 同学 DAVID_BLIP2(768) | **0.3755** | 0.3681 | 0.3708 | 最强（非 UNI 编码器） |

**归因结论**：同一 MLP 头下，换特征 PCC 差 ~+0.05（BLIP2 0.3755 vs 我们 CLS 0.324）；同学 UNI(3072)≈ 我们 Local+Global/忠实特征（0.368）——**增益来自表示(更强编码器/加 Local)，非 MLP 结构**。三种 MLP 头同特征只差 ~0.005–0.007。
