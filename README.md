...
| **GHIST** | 0.3164 | 0.2952 | — | — | 0.500 | 0.525 | 0.516 | 0.700 | ⏳ 重训中（batch8，~ep7 val 0.27） |
...
**本轮补齐（2026-09-06）**：SQUALL(解码器头) / BLEEP(冻结) / Hist2ST(官方配置) 三行的 unf **全评测**已补上（含 cell_PCC / SSIM / 全 Top-k）。为无固定种子单次运行；BLEEP 与 Hist2ST 另做 **seed 0/1 固定种子复跑**（`scripts/run_seed_rerun.sh`，GHIST 完成后启动）以核查能否回到旧 ~0.21 区间，完成后回填替换本表；GHIST(batch8) 仍在训练。
