...
10 方法，不同颜色）。完整逐 k 数值在各 `outputs/bench_*_f/eval_metrics.csv`。

**待完成**：GHIST（cpfs GPU OOM 修复后补跑）、Local+Global（过滤后调参 + 消融）。
**主要发现**：除 Hist2ST（−0.009）外**全部提升**，SQUALL 提升最大（+0.021）；
AUROC 普遍微降是"过滤去除 easy negatives 使二分类判别变难"的统计效应（见上节讨论），
不影响"过滤提升连续回归性能"的结论。
