...
> **STFlow / Phoenix 官方配置重跑（2026-08-28 进行中）**：按忠实度专项审查结论，
> 两者改用严格官方配置重跑（STFlow 官方去噪目标+超参+协议重训；Phoenix 微调改 zuko
> odeint 采样重测），结果出炉后更新本表。当前表内 STFlow 为本地适配版（0.0997）。
...
> STFlow、Phoenix 正按官方配置重跑中，暂不参与柱状图；出炉后更新。
