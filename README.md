...
**Top-k 曲线**：见 [`topk_accuracy_filtered.png`](topk_accuracy_filtered.png)（k=10..100，9 方法，
不同颜色）。完整逐 k 数值在各 `outputs/bench_*_f/eval_metrics.csv`。

**待完成**：BLEEP（测试中）、GHIST（cpfs GPU OOM 修复后补跑）、Local+Global（过滤后
调参 + 消融）。**主要发现**：除 Hist2ST（−0.009）外全部提升，SQUALL 提升最大（+0.021）。
