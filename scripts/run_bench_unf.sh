#!/bin/bash
# 最终 benchmark：rep1 → rep2（"未过滤"数据，用户 2026-08-29 决策）。
#
# 背景：用户核查后确认——"不在 H&E 上"的细胞指 rep2 已在 h5ad 阶段排除的那 6,835 个
# （Morphology FOV 底部条带，H&E y 翻转后覆盖不到）；rep1 无此现象。因此**不需要**
# 额外的 H&E 空白过滤，直接使用原 rep1(164,000)/rep2(111,345)。
#
# Phase 1：10 方法（除 Phoenix/SQUALL/LG）rep1 训练 → rep2 测试，单次运行。
# Phase 2（最后）：Local+Global 完整调参（op1 sweep → op2 sweep → 最终 50ep 训练+测试）。
#
# 评估：SSIM（空间表达图）+ 全 k=10..313 Top-k 曲线（topk_curve.csv）+ 误差棒数据
# （eval_metrics.csv_genes.csv 提供逐基因 PCC 的 mean±std）。
# 幂等：已存在 best.pt 的步骤自动跳过；LG sweep 各配置已存在特征/结果则跳过。
# 用法：nohup bash scripts/run_bench_unf.sh > logs/bench_unf_all.log 2>&1 &
set -u
cd "$(dirname "$0")/.."
...
say() { echo "[$(date '+%F %T')] $*" | tee -a logs/bench_unf_all.log; }

say "==== rep1→rep2 未过滤 benchmark 开始 ===="
...
# ---------- 1. UNI2+MLP（基线） ----------
m=uni2_mlp; D=outputs/bench_${m}_unf
if [ ! -f "$D/best.pt" ]; then
  say "== $m train =="
...
python3 scripts/test.py --method $m --ckpt $D/best.pt --test_dir data/rep2 \
  --gene_norm $GN --output_dir $D --device $DEV >> logs/bench_${m}_unf_train.log 2>&1
...
# ---------- 3. SpatialEx（整片超图） ----------
m=spatialex; D=outputs/bench_${m}_unf
if [ ! -f "$D/best.pt" ]; then
  say "== $m train =="
  python3 scripts/train.py --method $m --train_dir data/rep1 --valid_dir data/rep2 \
...
m=pixel2gene; D=outputs/bench_${m}_cell_unf
if [ ! -f "$D/best.pt" ]; then
  say "== $m cell train =="
  python3 scripts/train.py --method $m --variant cell --gene_norm log1p \
    ...
    --output_dir $D --device $DEV > logs/bench_${m}_cell_unf_train.log 2>&1
fi
say "== $m cell test =="
...
m=path2space; D=outputs/bench_${m}_unf
if [ ! -f "$D/best.pt" ]; then
  say "== $m train =="
  python3 scripts/train.py --method $m --train_dir data/rep1 --valid_dir data/rep2 \
    --epochs $EPOCHS --patience $PATIENCE --batch_size $BS --lr $LR --gene_norm $GN \
...
m=deeppt; D=outputs/bench_${m}_resnet50_unf
if [ ! -f "$D/best.pt" ]; then
  say "== $m ResNet50 train =="
  python3 scripts/train.py --method $m --feature_file X_resnet50.npy --feat_dim 2048 \
    --train_dir data/rep1 --valid_dir data/rep2 \
    --epochs $EPOCHS --patience $PATIENCE --batch_size $BS --lr $LR --gene_norm $GN \
    --output_dir $D --device $DEV > logs/bench_${m}_r50_unf_train.log 2>&1
fi
say "== $m ResNet50 test =="
...
python3 scripts/train.py --method $m --variant local_global \
  ...
  --output_dir $D --device $DEV > logs/bench_${m}_lg_unf_train.log 2>&1
