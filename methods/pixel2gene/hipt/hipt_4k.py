...
class HIPT_4K(torch.nn.Module):
    ...
    def forward_all256(self, x):
        ...
        # 2. einops "b c p1 p2 w h -> (b p1 p2) c w h" 等价原生（远程无 einops）
        batch_256 = batch_256.permute(0, 2, 3, 1, 4, 5).reshape(
            -1, batch_256.shape[1], 256, 256
        )  # [B x 3 x 256 x 256], where B = (1*w_256*h_256)
