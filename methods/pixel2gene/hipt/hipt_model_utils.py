...
def roll_batch2img(batch: torch.Tensor, w: int, h: int, patch_size=256):
    ...
    # einops "p1 p2 c w h -> c (p1 w) (p2 h)" 等价原生实现（远程无 einops）
    img = batch.permute(2, 0, 3, 1, 4).reshape(
        batch.shape[2], batch.shape[0] * batch.shape[3], batch.shape[1] * batch.shape[4]
    ).unsqueeze(dim=0)
