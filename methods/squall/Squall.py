...
# === 远程无 utils/sksurv：本地 stub（仅影响 logging/metrics，不影响模型架构） ===
def print_log(msg, logger='', **kwargs):
    print(f'[{logger}] {msg}' if logger else msg, flush=True)
def get_missing_parameters_message(missing_keys):
    return f'Missing keys ({len(missing_keys)}): {list(missing_keys)[:3]}'
def get_unexpected_parameters_message(unexpected_keys):
    return f'Unexpected keys ({len(unexpected_keys)}): {list(unexpected_keys)[:3]}'
def concordance_index_censored(*args, **kwargs):
    return (0.0, 0, 0)
from sklearn.metrics import accuracy_score, f1_score, roc_auc_score, r2_score
from scipy.stats import pearsonr
from sklearn.metrics.pairwise import cosine_similarity
from .transformer import Attention, MultiWayMLP, TransformerDecoder
